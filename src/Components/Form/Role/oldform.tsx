...
const CreateRoleForm: React.FC<Prop> = ({rname, description, roleid, disp}) => {
	const [fields, setFields] = useState<FieldData[]>([{name: ['roleid'], value: roleid}, { name: ['name'], value: rname },{ name: ['description'], value: description }]);
	...
	//Wait for 1 ms before setting data
	setTimeout(() => {
		form.setFieldsValue({
			name: rname,
			roleid: roleid,
		...
		<>
			<Form
				...
				onFieldsChange={(_, allFields) => {
					...
					form.setFieldsValue({
						name: allFields[1].value,
						description: allFields[2].value
					});
					console.log(form.getFieldValue("name").length);
