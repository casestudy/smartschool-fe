...
import { createAcademicYearAsync, modifyAcademicYearAsync } from '../../../State/Thunks/CalendarThunk';
...
const CreateAcademicYearForm: React.FC<Prop> = ({sdate, edate, description, yearid, disp}) => {
	const [fields, setFields] = useState<FieldData[]>([{name: ['yearid'], value: ''}, { name: ['startdate'], value: '' }, { name: ['enddate'], value: '' }, { name: ['descript'], value: ''}]);
	...
	useEffect(() => {
		if(typeof yearid !== 'undefined') {
			...
			if(!(fields[1].value > 0)) {
				//Don't read
				setTimeout(() => {
					setFields([{name: ['yearid'], value: yearid}, {name: ['sdate'] , value: sdate !== null? moment(sdate) : ''}, {name: ['edate'] , value: edate !== null? moment(edate) : ''}, {name: ['description'], value: description}]);
					setDisabled(false);
				},100);
			}
		} else {
			setTimeout(() => {
				setFields([{name: ['yearid'], value: ''}, {name: ['sdate'] , value: ''}, {name: ['edate'] , value: ''}, {name: ['description'], value: ''}]);
				setDisabled(false);
			},100);
		}
	},[])
	...
	const onFinish = () => {
		...
		if (data.yearid === '') {
			// We are adding
			setLoadingMessage('Creating academic year...');
			dispatch(createAcademicYearAsync(data)).then((value) => {
			...
			dispatch(modifyAcademicYearAsync(data)).then((value) => {
			...
			<Spin spinning={loading} tip={loadingMessage}>
				...
				>
                    ...
                    <Flex>
						<InputRow>
							<FormItem>
								<SaveButton title='Cancel' size='large' bgcolor='#8C8C8C' onClick={() => {navigate('/calendar')}}/>
							</FormItem>
							<FormItem>
